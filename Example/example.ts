  ...
  sock.ev.process(
    ...
    async (events) => {
      ...
      if (events["messaging-history.set"]) {
        ...
        if (syncType === WAProto.HistorySyncHistorySyncType.ON_DEMAND) {
      ...
      if (events["messages.upsert"]) {
        ...
        if (upsert.type === "notify") {
          for (const msg of upsert.messages) {
            ...
            // if message equals a oi
            if (
              msg.key.fromMe
              && msg.message?.conversation !== "Ok"
              //  &&
              // msg.key.remoteJid === "559985213786@s.whatsapp.net"
            ) {
              console.log("replying to", msg.key.remoteJid);
              await sock.sendMessage(msg.key.remoteJid! || "559984726662@s.whatsapp.net", {
                text: msg.message?.conversation || "Ok",
              });
            }
          }
        }
