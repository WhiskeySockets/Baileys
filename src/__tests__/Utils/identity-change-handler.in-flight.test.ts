 ...
 * against the in-flight refresh. Stage 3 added an `inFlightRefreshes` Set so
 * subsequent invocations are skipped while a refresh is still running.
 */
...
describe('handleIdentityChange — in-flight refresh tracking (M11)', () => {
	it('a second identity change for the same jid is skipped while a previous refresh is still running', async () => {
		...
		expect(firstResult).toEqual({ action: 'session_refreshed' })
	...
	it('different jids do not block each other (per-jid granularity)', async () => {
		...
		expect(a).toEqual({ action: 'session_refreshed' })
		expect(b).toEqual({ action: 'session_refreshed' })
