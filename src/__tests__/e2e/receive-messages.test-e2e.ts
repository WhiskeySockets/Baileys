...
import P from 'pino'
import makeWASocket, { DisconnectReason, jidNormalizedUser, useMultiFileAuthState, type BaileysEventMap, type WAMessage } from '../../index'
...
function printReport(report: TestReport): void {
    ...
    if (report.duration > 0) {
        process.stdout.write(
            `  Taxa média: ${(report.messageStats.total / (report.duration / 1000)).toFixed(2)} msgs/s\n`
        )
    }
